---
name: tdd-control-tower
description: 主 agent 控制面技能。以 Milestone 为单位派发与并行；用 data/dev-tasks.json 记录 Milestone 状态/依赖/工作区；派发 sub agent 使用 tdd-execution-worker 在同一 worktree 内完成“拆 Roadpoints + 逐点完成 + Milestone 整体合并到 main”。
---
...
多 Control Tower 共存规则（避免互相干扰）：
- 若上级要你“完成全部 Milestone”：你才对全量 `data/dev-tasks.json` 运行主循环，逐步把所有 Milestone 推到 DONE。
- 若上级只让你“完成其中一部分 Milestone”：你只处理被点名/明确指派的 Milestone 子集；其他 Milestone 不要 claim、不要改状态、不要分配 worktree（很可能由其他 control tower 负责）。
...
- 原验收 Milestone 先不要标记 `DONE`；等这些新 Milestone 完成后，再回来重跑验收。
- 验收结果至少要写清：通过项、问题清单、建议新增的 Milestone。
- 只有复验后没有新问题，验收 Milestone 才能 `DONE`。

### 5.2 调度范围（Scope）

主循环开始前先确定“本 control tower 的负责范围”：
...
3. 对每个要派发的 Milestone，`update`：
   ...
   - 若未分配则写 `worktree_dir/branch`（保证可复用）
   - `status=RUNNING`，写 `claimed_by/status_changed_at/updated_at`
4. 派发 sub agent，并明确要求其读取 worker skill：
   - Worker skill: `/Users/czj/.codex/skills/tdd-execution-worker/SKILL.md`
   - 任务单位：整个 Milestone（同一 sub agent 在同一 worktree 内完成 plan+execute）
   - 派发包必须包含 worker 的输入契约字段（尤其 `test_command`）
...
5. 监控：
   ...
   - 一轮监控建议做两件事：
     - 看产物是否在落盘：`TASKS/<milestone_id>-*.md`、`PROGRESS/<milestone_id>-*.md` 是否有新增/更新；Milestone 分支是否有新 commit；worktree 是否有新增文件。
     - 没有的话，再 ping sub agent（可选）：询问“当前 Roadpoint/卡点/下一步/预计多久给下一次落盘产物”。
   - 若没有产物落盘且 ping 无任何回应：再等待一轮（让出时间给它继续工作）。
   - 只有当“至少连续 2 轮没有任何落盘产物 + 多次 ping 无回应”时，才认为 sub agent 可能死亡：
     - 先关闭该 sub agent
